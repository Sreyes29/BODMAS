...
def eval(expression, position):  
    result = 0
    reducedexp = ''
    token, nextposition = findtoken(expression, position) #Finding the first token of the expression along with its position
    ntoken, nnextposition = findtoken(expression, nextposition) #Finding the second token of the expression along with its position
    if ntoken == '?': #Last token condition
        return token
    nntoken, nnnextposition = findtoken(expression, nnextposition) #Finding the third token of the expression along with its position
    if ntoken == '/' or ntoken == '*':
        if ntoken == '/':
